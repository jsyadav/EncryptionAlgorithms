...
void rijndaelEncryptRound(const u32 rk[/*4*(Nr + 1)*/], int Nr, u8 block[16], int rounds) {
    int r;
    u32 s0, s1, s2, s3, t0, t1, t2, t3;
//printf("rk[20]="); for(int i =0;i<20;i++) printf("%d-",rk[i]);printf("\n");
//printR("block", block, 16);
    /*
	 * map byte array block to cipher state
	 * and add initial round key:
	 */
...
void printE(char *msg, unsigned char* buf, int len){
/*
    printf("%s[%d]", msg, len);
    for (int i = 0;i < len;i++){
      printf("%02x",buf[i]);
...
static void E(byte *K, unsigned kbytes, int j, unsigned i,
    ...
    //printE("Exe Key", extracted_key, 48);
...
extern "C"
jint
Java_com_example_jyadav_encryptionalgorithms_MainActivity_aez_1crypto_1aead_1encrypt(
        ...
        jbyteArray nsec, jbyteArray npub, jbyteArray k)
{
    ...
    jint i =  aez_crypto_aead_encrypt(
...
extern "C"
jint
Java_com_example_jyadav_encryptionalgorithms_MainActivity_aez_1crypto_1aead_1decrypt(
        ...
        jbyteArray npub, jbyteArray k)
{
    ...
    jint i =  aez_crypto_aead_decrypt(
