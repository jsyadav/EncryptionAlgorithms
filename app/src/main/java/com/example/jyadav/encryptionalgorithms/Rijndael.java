...
public class Rijndael {
	...
	{
		return (((int) buf[3+  offset] & 0xFF ) ^ 
	    (((int) buf[2+offset] & 0xFF) <<  8) ^  
	    (((int) buf[1+offset] & 0xFF) << 16) ^ 
	    (((int) buf[0+offset] & 0xFF) << 24));			    
	}
	private static int GETU32_1(byte[] buf, int offset)
	{
		return (((int) buf[offset] & 0xFF ) ^ 
		    (((int) buf[1+offset] & 0xFF) <<  8) ^  
		    (((int) buf[2+offset] & 0xFF) << 16) ^ 
		    (((int) buf[3+offset] & 0xFF) << 24));
	}
	...
	public static void rijndaelEncryptRound(byte[/*4*Nr+1*/] rkb, int Nr, byte[] block , int rounds){
	     int s0, s1, s2, s3, t0, t1, t2, t3;
	     int r, rkoffset,ptoffset;
	     ptoffset= 0;
	     int [] rk = new int[rkb.length/4];
	     for (int i=0;i<rkb.length/4;i++) {
	    	 rk[i] = GETU32_1(rkb,i*4);
	     }
	     Aezv4.print("rk", rk, rkb.length/4, 0);
	     Aezv4.print("block", block, block.length, 0);
	     /*
	      * map byte array block to cipher state
	      * and add initial round key:
	      */
	     s0 = GETU32(block, ptoffset     ) ^ rk[0];
	     s1 = GETU32(block, ptoffset +  4) ^ rk[1];
	     s2 = GETU32(block, ptoffset +  8) ^ rk[2];
	     s3 = GETU32(block, ptoffset + 12) ^ rk[3];
	      ...
	      */
		    ...
		    for (r = (rounds < Nr ? rounds : Nr - 1); r > 0; r--) {
                ...
                t3 =
                        ...
                        Te3[(s2      ) & 0xff] ^
                        rk[rkoffset+3];
                //System.out.println("t0="+t0+",t1="+t1+",t2="+t2+",t3="+t3);
	        ...
	        PUTU32(block, 0+ptoffset, s0);
	        PUTU32(block, 4+ptoffset, s1);
	        PUTU32(block, 8+ptoffset, s2);
	        PUTU32(block, 12+ptoffset, s3);
